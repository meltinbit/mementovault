...
export function TokenManager({ tokens, collectionId, mcpEndpoint, newToken: initialNewToken }: TokenManagerProps) {
    ...
    const generateToken = () => {
        post(route('collections.tokens.store', collectionId), {
            onSuccess: (page: any) => {
                const token = (page.props as any).newToken;
                if (token) {
                    setNewToken(token);
                }
                setData('name', '');
    ...
    const revokeToken = () => {
        ...
        router.delete(route('collections.tokens.destroy', [collectionId, deleteToken.id]), {
        ...
        <div className="space-y-4">
            <div className="flex items-end gap-3">
                <div className="grid flex-1 gap-1.5">
                    ...
                    <Input id="token-name" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="default" className="h-9" />
                </div>
            ...
            {tokens.length === 0 && !newToken ? (
                <p className="py-4 text-center text-sm text-muted-foreground">No tokens yet. Generate one to connect via MCP.</p>
            ) : (
            ...
            <DeleteConfirmation
                ...
                description="Any clients using this token will lose access."
            />
        </div>
    );
}
